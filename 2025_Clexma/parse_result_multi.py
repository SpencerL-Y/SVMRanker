...
C_BOOGIES_FOLDER = os.path.join(os.getcwd(), "C_Boogies")
PROGRAM_MULTI_FOLDER = os.path.join(os.getcwd(), "Program_multi")
SIFTED_TERMINATE_FOLDER = os.path.join(os.getcwd(), "2025_Clexma", "Multi", "4-multi-terminate")

def sift_terminate_files(terminate_list):
    for item in terminate_list:
        fname = item['filename']
        depth = item['multi_phase_depth']
        original_file = os.path.join(PROGRAM_MULTI_FOLDER, fname)
        print("cp " + original_file + " " + os.path.join(SIFTED_TERMINATE_FOLDER, str(depth) + "_" + fname))
...
def parse_file(path):
    ...
    multi_depth = None
    if result == "TERMINATE":
        depth_m = re.search(r"MULTIPHASE DEPTH:\s*(\d+)", text)
        if depth_m:
            multi_depth = int(depth_m.group(1))

    return {
        'filename': os.path.basename(path),
        'result': result,
        'multi_phase_depth': multi_depth,
        'time': time_val
    }
...
def main():
    ...
    p.add_argument(
        '--json', '-j',
        action='store_true',
        help="以 JSON 格式输出（仅 TERMINATE，且提取 core filename 和 multi_phase_depth）"
    )
    args = p.parse_args()
    ...
    if args.json:
        ...
        for r in terminate_only:
            ...
            cleaned.append({
                "filename": core,
                "multi_phase_depth": r["multi_phase_depth"]
            })
        ...
        print("-"*60)
        for r in results:
            depth = r['multi_phase_depth'] if r['multi_phase_depth'] is not None else '-'
