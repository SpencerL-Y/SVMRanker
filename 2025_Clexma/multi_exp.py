...
if __name__ == "__main__":
    ...
    for file in os.listdir(BOOGIE_PROGRAM_FOLDER):
        if file.endswith("bpl"):
            command = ["python3", os.path.join(PROJECT_ROOT, "src", "CLIMain.py"), "lmulti", "--depth_bound 4", os.path.join(BOOGIE_PROGRAM_FOLDER, file), ">>", os.path.join(RESULT_FOLDER, "output_" + file + ".txt")]
    ...
    for file in os.listdir(C_BOOGIE_PROGRAM_FOLDER):
        if file.endswith("bpl"):
            command = ["python3", os.path.join(PROJECT_ROOT, "src", "CLIMain.py"), "lmulti", "--depth_bound 4", os.path.join(BOOGIE_PROGRAM_FOLDER, file), ">>", os.path.join(RESULT_FOLDER, "output_" + file + ".txt")]
